...
app.post("/xacnhandattiec", async (req, res) => {
  ...
  const thangdattiec = date.getMonth();
  ...
  tongtienban += dongiaban * (parseInt(soluongban) + parseInt(sobandutru));
...
app.get("/haha", async (req, res) => {
  const abc = await db.query(
    "SELECT * FROM PHIEUDATTIEC WHERE tenchure = $1 AND tencodau = $2",
    ["Võ Vũ Trường Giang", "Trần Thị Nhã Phương"]
  );
  let sophieu = abc.rows[0];
  console.log(sophieu);
  res.redirect("/main");
